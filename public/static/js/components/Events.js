        ...
        return ( <
            ...
            /div> {
                eventsList.map((value, index) => ( <
                    ...
                    <
                    div className = {
                        `bg-success me-2 p-3 rounded-xxl ${value.status}`
                    } > < h4 className = "fw-700 font-lg ls-3 lh-1 text-white mb-0" > < span className = "ls-1 d-block font-xsss text-white fw-600" > {
                        value.month
                    } < /span>{value.date}</h
                    4 > < /div> <
