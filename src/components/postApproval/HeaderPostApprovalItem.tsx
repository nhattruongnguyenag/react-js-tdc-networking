        ...
        <>
            <div className='card-body d-flex w-100 m-0 p-0'>
                ...
                <div className='text-header-name-wrapper'>
                    <h4 className='cursor-pointer fw-700 text-grey-900 font-xssss mt-1'
                        onClick={() => handleClickToAvatarAndName()}>
                        {props.post?.user.name}
                    </h4>
                    <div className='flex flex-row mt-1'>
                        <span className="d-block font-xssss fw-500 lh-3 text-grey-500 mt-1">{props.post?.createdAt ? moment(props.post.createdAt).fromNow() : t('ModalPostRejectReason.isLoading')}</span>
                        <span className={classNames("px-3 rounded-full text-sm ms-2", badge.color)}>{t(badge.content)}</span>
                    </div>
                </div>
