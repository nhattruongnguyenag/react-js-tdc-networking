...
import { AVATAR_CLICK, AVATAR_CLICK_UPLOAD, BACKGROUND_CLICK, BACKGROUND_CLICK_UPLOAD, CALL_ACTION, CHANGE_INFO_USER_CLICK_ACTION, CLICK_CAMERA_AVATAR_EVENT, CLICK_CAMERA_BACKGROUND_EVENT, FOLLOW_ACTION, MENU_CLICK_ACTION, MESSENGER_ACTION, SEE_AVATAR, SEE_BACKGROUND } from '../constants/Variables'
...
export default function UserDetailsPage() {
  ...
  const handleSelectImageToUpload = async (isBackground: boolean, flag: boolean) => {
    if (flag) {
      if (isBackground) {
        const status = await updateImageUserProfile(SERVER_ADDRESS + "api/users/change/image", { ...userInformationChangeBackground });
        printfNotifications(status);
      } else {
        const status = await updateImageUserProfile(SERVER_ADDRESS + "api/users/change/image", { ...userInformationChangeAvatar });
        printfNotifications(status);
  ...
  const printfNotifications = (status: number) => {
    status === 200 ? toast.success(t("Toast.toastUpdateProfileSuccess")) : toast.success(t("Toast.toastUpdateProfileUnSuccess"))
  }
            ...
            <div className='row feed-body'>
              {
                isLoading ?
                  ...
                  : <div className='col-12'>
                    ...
                    {
                      ((Boolean(group)) && Boolean(getGroupForPost(group, t))) && <div className='card w-100 shadow-xss rounded-xxl mb-3 mt-3 border-0 p-4 '>
                        <div className='snippet mt-2 wrapperTitleUserProfile' data-title='.dot-typing'>
                          <span className='txtTitleInUserProfile text-grey-900 text-dark'>
                            ...
                            <FontAwesomeIcon className='iconArrowToRightUserProfile' icon={faPlay} size='1x' color={COLOR_GREY} />
